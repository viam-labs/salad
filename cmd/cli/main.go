...
func init() {
	...
	grabCmd.Flags().IntVar(&grabFlags.MaxDepthPositions, "max-depth-positions", 3, "number of distinct depth levels to try before abandoning a target")
	grabCmd.Flags().Float64Var(&grabFlags.WeightThresholdG, "weight-threshold-g", 1.0, "minimum weight gain (g) to consider a grab successful")
	grabCmd.Flags().Float64Var(&grabFlags.TargetWeightG, "target-weight-g", 0.0, "stop grabbing once this total weight (g) has been deposited on the scale (0 = grab until exhausted)")
	...
	grabCmd.Flags().Float64Var(&grabFlags.ScaleApproachHoverMM, "scale-approach-hover-mm", 400.0, "transit height (mm) above scale surface when traveling to/from scale")
	grabCmd.Flags().Float64Var(&grabFlags.ScaleDropHoverMM, "scale-drop-hover-mm", 250.0, "drop height (mm) above scale surface when opening gripper")
	grabCmd.Flags().Float64Var(&grabFlags.OrientationOX, "orientation-ox", 0.0, "OX component of grab orientation vector")
	grabCmd.Flags().Float64Var(&grabFlags.OrientationOY, "orientation-oy", 0.0, "OY component of grab orientation vector")
	grabCmd.Flags().Float64Var(&grabFlags.OrientationOZ, "orientation-oz", -1.0, "OZ component of grab orientation vector")
	grabCmd.Flags().Float64Var(&grabFlags.OrientationTheta, "orientation-theta", -90.0, "Theta (degrees) of grab orientation vector")
	...
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.InitialDepthMM, "initial-depth-mm", 0.0, "how far below the bin rim (mm) the first grab attempt starts")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.FloorBufferMM, "floor-buffer-mm", 50.0, "minimum clearance (mm) above the bin floor on the deepest attempt")
	grabRandomCmd.Flags().IntVar(&grabRandomFlags.MaxDepthPositions, "max-depth-positions", 8, "number of distinct depth positions evenly spaced from first grab Z down to floor buffer")
	grabRandomCmd.Flags().IntVar(&grabRandomFlags.GrabSettleMs, "grab-settle-ms", 500, "milliseconds to wait after descent and after Grab() before retreating")
	grabRandomCmd.Flags().IntVar(&grabRandomFlags.ScaleSettleMs, "scale-settle-ms", 100, "milliseconds to wait after opening gripper before reading scale weight")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.MinDepositG, "min-deposit-g", 2.0, "minimum weight change (g) to consider as a real deposit and retry at the same depth")
	...
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.TargetWeightG, "target-weight-g", 0.0, "stop grabbing once this total weight (g) has been deposited on the scale (0 = grab until exhausted)")
	...
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.ScaleApproachHoverMM, "scale-approach-hover-mm", 400.0, "transit height (mm) above scale surface when traveling to/from scale")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.ScaleDropHoverMM, "scale-drop-hover-mm", 250.0, "drop height (mm) above scale surface when opening gripper")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.OrientationOX, "orientation-ox", 0.0, "OX component of grab orientation vector")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.OrientationOY, "orientation-oy", 0.0, "OY component of grab orientation vector")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.OrientationOZ, "orientation-oz", -1.0, "OZ component of grab orientation vector")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.OrientationTheta, "orientation-theta", -90.0, "Theta (degrees) of grab orientation vector")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.GripperLengthMM, "gripper-length-mm", 0.0, "length (mm) of gripper/scoop attachment; used to compensate XY position for orientation tilt")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.TipClearanceMM, "tip-clearance-mm", 30.0, "minimum clearance (mm) between scoop tip and bin floor (used when --gripper-length-mm is set)")
	grabRandomCmd.Flags().Float64Var(&grabRandomFlags.DescentOffsetMM, "descent-offset-mm", 0.0, "XY descent offset (mm) in the opposite direction of tilt; overrides the auto-computed value (OX/mag * gripper-length-mm)")
