...
func runHeightMap(flags HeightMapFlags) error {
	...
	for i := range targets {
		stats, _ := segmentation.ZonePlaneFitStats(pc, &targets[i], nil)
		if i > 0 {
			fmt.Fprintln(os.Stdout)
		}
		printHeightMapGrid(os.Stdout, targets[i], stats)
	}
	return nil
}
...
func printHeightMapGrid(w io.Writer, zone segmentation.Zone, stats segmentation.PlaneFitStats) {
	hm := stats.HeightMap
