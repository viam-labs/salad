...
func runSegment(flags SegmentFlags) error {
	...
	if flags.Viz {
		if err := drawFullZonesViz(zoneVizOptions{ClearFirst: true, VizURL: flags.VizURL, LocalDir: filepath.Dir(flags.MeshPath), PreferredMesh: flags.MeshPath, Result: result}); err != nil {
			return err
		}
		logger.Infof("Sent %d zone(s) to %s", len(result.Zones), flags.VizURL)
	}
	return nil
}
